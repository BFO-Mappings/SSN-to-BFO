#!/usr/bin/env python3
"""Clean-source detached-checkout SOSA-2023 release rehearsal."""
...
from sosa_2023_build_release import (
    PACKAGE_FILE_PATHS,
    compare_complete_packages,
)
from sosa_2023_release_runtime import resolve_validation_toolchain
from sosa_2023_check_release import _expected_directories
from sosa_2023_release_archive import (
...
from sosa_2023_release_manifest import ReleaseManifest, load_and_validate_release_manifest

if TYPE_CHECKING:
    from sosa_2023_release_runtime import ResolvedValidationToolchain
...
def _build_candidate(
    ...
    _run_candidate_phase(
        "PACKAGE_BUILD_FAILED",
        "package",
        [python, "-B", "tools/sosa_2023_build_release.py", *common, "--notes", notes_relative, "--output-dir", str(package)],
        checkout=checkout,
    ...
    _run_candidate_phase(
        ...
        "package",
        [python, "-B", "tools/sosa_2023_check_release.py", "--package-dir", str(package)],
        checkout=checkout,
        phase_environment=environment,
        git_environment=git_environment,
        source_commit=source_commit,
...
def _validate_staged_output(
    ...
    _phase_command(
        ...
        "output_dir",
        [sys.executable, "-B", "tools/sosa_2023_check_release.py", "--package-dir", str(staged_package)],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(str(exc))
        return 1
    print("SOSA-2023 release rehearsal: PASS")
