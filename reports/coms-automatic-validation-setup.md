...
The SOSA-next track is separately active as a maintained development track. `src/sosa-next/catalog-v001.xml` resolves its pinned source closure, track-specific merged CCO/BFO dependency at `src/sosa-next/imports/cco.ttl`, Integrated Mapping, BFO Mapping, CCO Extension, and editor shell. The editor imports Integrated; the modular CCO Extension imports only BFO Mapping; BFO Mapping is import-free. The SOSA-next generator and checker enforce deterministic products, exact import boundaries, a 268-triple Integrated mapping graph isomorphic to the BFO+CCO modular union, product-specific reasoning closures of 15,231, 15,114, and 15,239 triples, and a 285-triple catalog-resolved editor project stack. This development activation does not add SOSA-next products to the current SSN/SOSA formal package; SOSA-2023 uses separate formal release machinery.
