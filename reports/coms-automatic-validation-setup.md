...
`config/publication-metadata.toml` is the sole governed publication-metadata source. Schema version 3 validates the exact approved global values, both authority-status IRIs, and the canonical five-product order, paths, ontology IRIs, release suffixes, labels, descriptions, and product-type IRIs. The loader rejects missing or unknown tables and fields, unsafe paths, malformed IRIs, duplicate identities, and noncanonical text.
...
The COMS freshness transaction hashes the metadata source, so a metadata edit makes all generated evidence stale until the normal transaction succeeds. The development transaction loads schema-3 metadata once and deterministically emits the same ordered seven-predicate development metadata model in the integrated root and all four modules: `rdfs:label`, `dcterms:description`, `dcterms:type`, `adms:status`, `dcterms:license`, `rdfs:seeAlso`, and `rdfs:comment`. Exact RDF term kinds, `@en` tags, ontology subjects, imports, and the absence of release-only fields are validated before publication.

`make check-release-rendering` runs the release-context, formal-rendering, publication-metadata, generator, and modular-product regressions. For the clearly synthetic `2099-01-02` context, formal rendering keeps stable ontology subjects, substitutes the immutable-release status, appends `owl:versionIRI`, plain `owl:versionInfo`, and typed `dcterms:issued`, and rewrites only project imports to same-release version IRIs. It independently reasons over all five formal products; it does not create a dated directory, package, manifest, archive, tag, or published release.

`make check-release-package` runs the release-manifest and package tests plus the release-context and formal-rendering preservation regressions. `tools/build_release.py` creates a deterministic candidate only at an explicit absent temporary output path, and `tools/check_release.py validate --package-dir PATH` independently checks it read-only. The exact 13-file layout contains the five formal TTLs, version-IRI-only OASIS catalog, schema-1 canonical manifest, `SHA256SUMS`, approved release notes, project license, governed workbook and metadata, and product-disposition evidence. External ontology dependencies are recorded by identity and hash but are not redistributed, so only project-module imports resolve wholly inside the package.

The manifest excludes itself and `SHA256SUMS` from `included_files`; `SHA256SUMS` includes `manifest.json` and excludes only itself, with lowercase hashes, two spaces, lexicographic relative paths, and one final newline. Fixed inputs and context reproduce all 13 bytes across fresh processes. Package output is never overwritten. The current tooling validates source-commit syntax but does not bind it to a clean checkout, build an archive, choose actual release identity or notes, create a tag or GitHub release, clean placeholders, or deploy persistent IRIs.

Formal direct totals are integrated 1,127, alignment core 64, strict BFO 137, import-only BFO projection 12, and CCO extension 946. The formal alignment-core source closure has 1,215 triples; strict BFO has 14,992; projection project graphs have 213/211 triples and its fixed closure has 15,003; CCO project graphs have 1,147/1,145 and its fixed closure has 15,937; and the integrated fixed closure has 15,915. These increases are exactly the three formal metadata triples per product plus same-release project import substitution.
